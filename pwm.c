...
void pwm_disable(pwm_channel_id_t ch, gpio_id_t pin) {
    // Ensure the channel is valid
    assert(ch >= PWM0 && ch <= PWM7);

    // Clear the enable bit in the 'per' register for the specified channel
    module.pwm->regs.per &= ~(1 << ch);

    // Drive the GPIO pin low to simulate a power disconnect
    gpio_set_output(pin);
    gpio_write(pin, 0); // Force the pin to LOW
}


void pwm_enable(pwm_channel_id_t ch) {
    // Ensure the channel is valid
    assert(ch >= PWM0 && ch <= PWM7);

    // Set the enable bit in the 'per' register for the specified channel
    module.pwm->regs.per |= (1 << ch);
}
